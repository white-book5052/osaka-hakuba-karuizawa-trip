// 2泊3日旅行費用計算プログラム（軽井沢2泊・アウトレット・中央道不使用版）
function 旅行費用計算_2泊3日_軽井沢() {
  // 基本情報
  const 人数 = 3; // 大人3人
  const 宿泊数 = 2; // 2泊

  // 交通費
  const 総走行距離km = 1200; // 往復合計の推定距離（中央道を使わないルート）
  const ガソリン代_1km = 20; // 1kmあたりの燃料代（円）
  const 高速道路料金_往復 = 32000; // 往復の高速料金
  ...
  const 朝食_涼の音 = 3000; // 旧軽井沢Cafe涼の音での朝食（3人分）
  const 観光費_合計 = 5000; // 入場料や体験費用の合計
  ...
  const 宿泊費 = (軽井沢マリオット_1泊 * 2) + (ペット追加料金 * 2); // 軽井沢に2泊
  const 食事代 = (食事代_1人1日 * 人数 * 3) + 朝食_涼の音; // 3日分 + 特別朝食

  // 合計
  const 総費用 = ガソリン代 + 高速道路料金_往復 + 宿泊費 + 食事代 + 観光費_合計 + 犬関連費用 + 温泉入浴料;

  // 結果表示
  console.log("【2泊3日旅行費用見積もり（軽井沢2泊・アウトレット・中央道不使用版）】");
  console.log(`ガソリン代: ${ガソリン代.toLocaleString()}円`);
  console.log(`高速道路料金: ${高速道路料金_往復.toLocaleString()}円`);
  console.log(`宿泊費: ${宿泊費.toLocaleString()}円（軽井沢マリオット2泊）`);
